...
tab.corr <- tabPanel(tags$b("Correlation"),
             ...
             plotOutput("scatter", height = '600px', width = '90%'),
...
tab.clust <- tabPanel(tags$b("Clustering"),
                      ...
                      plotOutput("heatmap", height = '400px', width = '90%'), br(),
                      cap.clust, br()
                      )
...
ui <- fluidPage(theme = shinytheme("lumen"), #paper is good, flatly, journal
    sidebarLayout(
        ### Sidebar
        sidebarPanel(hr,hr(),
        ...
        ### Main
        mainPanel(tabsetPanel(tab.ts, tab.cyclic, tab.ACF,
                                    tab.corr, tab.clust),
                  br(),br(),br(),br())
    )
)
    ...
    output$heatmap <- renderPlot({
        matrix <- bikes %>% as_tibble() %>%
            ...
            tidyr::pivot_wider(id_cols = "Date",
                        ...
                        values_from = 'count') %>%
            mutate_if(is.character, as.numeric) %>%
            select(-c(1,13,14)) %>%  # make sure date isn't in matrix
            as.matrix()

        coul <- viridis_pal(alpha = 1, begin = 0, end = 1, direction = 1,
                            option = "A")(1000)
        b <- rcorr(matrix)
        heatmap.2(b$r, scale="column",
                  trace='none', col = coul,
